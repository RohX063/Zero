...
void initAttacks() {
    for(int sq = 0; sq < 64; sq++) {
        ...
        U64 atkKnight = 0;
        atkKnight |= ((b << 17) & ~FILE_A);
        atkKnight |= ((b << 15) & ~FILE_H);
        atkKnight |= ((b << 10) & ~(FILE_A | (FILE_A << 1)));
        atkKnight |= ((b << 6) & ~(FILE_H | (FILE_H >> 1)));
        atkKnight |= ((b >> 17) & ~FILE_H);
        atkKnight |= ((b >> 15) & ~FILE_A);
        atkKnight |= ((b >> 10) & ~(FILE_H | (FILE_H >> 1)));
        atkKnight |= ((b >> 6) & ~(FILE_A | (FILE_A << 1)));
        knightAttacks[sq] = atkKnight;
        U64 atkKing = 0;
        atkKing |= (b << 8);
        atkKing |= (b >> 8);
        atkKing |= ((b << 1) & ~FILE_A);
